...
-- =============================================================================
-- This migration fixes the session hijacking vulnerability with correct PostgreSQL syntax.
...
-- 8. Create admin session monitoring trigger (corrected syntax)
...
AS $$
BEGIN
  -- Log all admin session access attempts
  INSERT INTO public.security_events (
    event_type, severity, event_data, source, ip_address
  ) VALUES (
...
-- Apply monitoring trigger (corrected syntax - no SELECT trigger)
...
-- 10. Create additional admin authentication verification function
CREATE OR REPLACE FUNCTION public.verify_admin_access_with_session_check()
RETURNS BOOLEAN
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = ''
...
BEGIN
  -- Check if user is admin and has active sessions
  IF auth.uid() IS NOT NULL AND has_role(auth.uid(), 'admin') THEN
    -- Count active sessions for this admin
    ...
    is_valid_admin := active_sessions > 0;
    ...
    INSERT INTO public.security_events (
      ...
      auth.uid(),
      jsonb_build_object(
        'verification_result', is_valid_admin,
...
INSERT INTO public.security_events (
  ...
  jsonb_build_object(
    'security_fixes_applied', jsonb_build_array(
      ...
      'admin_verification_enhanced'
    ),
    'implementation_timestamp', now(),
    'security_level', 'military_grade',
    'vulnerabilities_patched', jsonb_build_array(
      'session_hijacking_prevention',
      'unauthorized_admin_access_prevention',
      'cms_content_protection',
      'seo_manipulation_prevention',
      'lead_data_theft_prevention'
    )
  ),

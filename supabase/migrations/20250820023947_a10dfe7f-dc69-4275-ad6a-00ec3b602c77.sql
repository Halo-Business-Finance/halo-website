-- Security Fix Migration: Address Critical Role Escalation Vulnerability
-- This migration implements comprehensive security fixes

-- Step 1: Remove the dangerous RLS policy that allows users to update their own roles
DROP POLICY IF EXISTS "Only admins can manage roles with validation" ON public.user_roles;

-- Step 2: Create a more secure RLS policy that prevents self-role modification
CREATE POLICY "Secure admin role management only" 
ON public.user_roles 
FOR ALL 
USING (
  has_role(auth.uid(), 'admin'::app_role) 
  AND auth.uid() IS NOT NULL 
  AND auth.uid() != user_id  -- Prevent self-modification
)
WITH CHECK (
  has_role(auth.uid(), 'admin'::app_role) 
  AND auth.uid() IS NOT NULL 
...
-- Step 3: Create a secure role change validation trigger
...
CREATE TRIGGER validate_role_changes
  ...
  FOR EACH ROW EXECUTE FUNCTION public.validate_secure_role_changes();

-- Step 4: Create secure admin initialization function
...
-- Step 5: Enhance the secure role assignment function
...
-- Step 6: Add session security enhancements

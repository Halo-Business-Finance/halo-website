...
CREATE POLICY "Users can view own consultations with data masking" 
...
ADD COLUMN last_security_check timestamp with time zone DEFAULT now();

-- Create more restrictive session access policy
...
-- Add triggers for security data access logging
CREATE TRIGGER audit_security_events_access
  AFTER SELECT OR INSERT OR UPDATE OR DELETE ON public.security_events
...
-- Create enhanced session security validation
CREATE OR REPLACE FUNCTION public.validate_secure_session_access(session_id uuid)
RETURNS boolean
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO ''
...
BEGIN
  ...
  IF NOT FOUND THEN
    -- Log suspicious session access attempt
    INSERT INTO public.security_events (
      event_type, severity, user_id, ip_address, event_data, source
    ) VALUES (
      'suspicious_session_access', 'high', auth.uid(), inet_client_addr(),
      jsonb_build_object(
        'session_id', session_id,
        'reason', 'session_not_found_or_compromised'
      ),
      'session_security'
    );
    RETURN false;
  ...
  INSERT INTO public.security_events (
    ...
    'secure_session_access_validated', 'info', auth.uid(),
    jsonb_build_object(
      'session_id', session_id,
      'access_count', session_data.access_count + 1
    ),
    'session_security'
  );

-- Create enhanced database-level PII protection and validation

-- First, add a function to mask sensitive data for display
...
-- Create secure consultation insertion function
CREATE OR REPLACE FUNCTION public.create_secure_consultation(
  p_encrypted_name text,
  p_encrypted_email text,
  p_encrypted_phone text DEFAULT NULL,
  p_company text DEFAULT NULL,
  p_loan_program text,
  p_loan_amount text DEFAULT '',
...
BEGIN
  ...
  IF current_user_id IS NULL THEN
    RAISE EXCEPTION 'Authentication required for consultation creation';
  END IF;
  
  -- Validate required encrypted fields
  IF p_encrypted_name IS NULL OR p_encrypted_email IS NULL OR p_loan_program IS NULL THEN
    RAISE EXCEPTION 'Required fields missing';
  END IF;
  
  -- Insert consultation with proper validation
  ...
  -- Log secure consultation creation
  INSERT INTO public.security_events (
    event_type, severity, user_id, event_data, source
  ) VALUES (
    'secure_consultation_created', 'info', current_user_id,
    jsonb_build_object(
      'consultation_id', consultation_id,
      'loan_program', p_loan_program,
      'has_company', p_company IS NOT NULL,
      'has_phone', p_encrypted_phone IS NOT NULL,
      'creation_method', 'secure_function'
    ),
    'secure_database_function'
  );

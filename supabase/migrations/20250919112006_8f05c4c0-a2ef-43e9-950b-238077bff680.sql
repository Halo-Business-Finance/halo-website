...
BEGIN
  ...
  IF master_key_id IS NULL THEN
    RAISE EXCEPTION 'No active encryption key found for lead data';
  END IF;
...
BEGIN
  ...
  IF financial_key_id IS NULL THEN
    RAISE EXCEPTION 'No active financial encryption key found';
  END IF;
...
-- 5. Create master encryption keys for the new system
-- ============================================================================

-- Insert master encryption keys
...
(
  ...
  encode(digest(encode(gen_random_bytes(64), 'hex') || 'LEAD_MASTER_2025', 'sha512'), 'hex'),
...
(
  ...
  encode(digest(encode(gen_random_bytes(64), 'hex') || 'FINANCIAL_MASTER_2025', 'sha512'), 'hex'),
  encode(gen_random_bytes(32), 'hex'),
  'HALO-AES-EQUIV-2025',
  now() + interval '1 year'

...
AS $$
BEGIN
  -- Log all access to business application data
  INSERT INTO public.security_events (
    ...
    CASE TG_OP
      WHEN 'SELECT' THEN 'business_data_accessed'
...
CREATE TRIGGER audit_business_applications
  AFTER SELECT OR INSERT OR UPDATE OR DELETE ON public.applications
  ...
  SELECT 
    ...
    CASE 
      WHEN caller_role = 'admin' THEN 
        public.mask_financial_data(a.loan_amount, caller_role)
      ELSE 'CONFIDENTIAL'
...
-- 8. Create scheduled job for data retention (commented for manual execution)
-- SELECT cron.schedule('data-retention-job', '0 2 * * 0', 'SELECT public.enforce_data_retention();');

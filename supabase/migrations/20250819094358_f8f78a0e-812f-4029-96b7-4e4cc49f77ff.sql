...
-- 1. Add enhanced service role protection policies
...
-- Create trigger for role assignment security
DROP TRIGGER IF EXISTS prevent_role_self_assignment ON public.user_roles;
CREATE TRIGGER prevent_role_self_assignment
  BEFORE INSERT ON public.user_roles
  FOR EACH ROW
  EXECUTE FUNCTION public.prevent_role_self_assignment();

-- 3. Fix security audit protection
DROP POLICY IF EXISTS "System can insert audit logs" ON public.security_access_audit;
CREATE POLICY "System functions only can insert audit logs"
ON public.security_access_audit
FOR INSERT
WITH CHECK (
  -- Only allow inserts from security definer functions or service role
  (current_setting('role') = 'service_role') OR
  (current_setting('app.current_function', true) IS NOT NULL)
);

-- 4. Enhanced PII protection functions
...
-- 5. Enhanced rate limiting with behavioral scoring
...
-- 6. Create secure consultation access function with enhanced logging

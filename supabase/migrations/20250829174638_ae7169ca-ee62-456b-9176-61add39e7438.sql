...
-- 1. Fix Consultation Data Security Gaps - Make encryption fields NOT NULL
ALTER TABLE public.consultations 
ALTER COLUMN encrypted_name SET NOT NULL,
ALTER COLUMN encrypted_email SET NOT NULL,
ALTER COLUMN encrypted_phone SET NOT NULL;

-- Add constraint to ensure encryption fields are not empty
ALTER TABLE public.consultations 
ADD CONSTRAINT check_encrypted_name_not_empty CHECK (encrypted_name != ''),
ADD CONSTRAINT check_encrypted_email_not_empty CHECK (encrypted_email != ''),
ADD CONSTRAINT check_encrypted_phone_not_empty CHECK (encrypted_phone != '');

-- 2. Strengthen Business Data Access Control - Enhanced RLS for applications
DROP POLICY IF EXISTS "Users can view their own applications" ON public.applications;
DROP POLICY IF EXISTS "Admins can view all applications" ON public.applications;

-- More restrictive application access policies
CREATE POLICY "Secure user application access" ON public.applications
FOR SELECT USING (
  auth.uid() = user_id AND auth.uid() IS NOT NULL
);

CREATE POLICY "Secure admin application access" ON public.applications  
FOR SELECT USING (
  has_role(auth.uid(), 'admin'::app_role) AND auth.uid() IS NOT NULL
);

-- Add field-level protection for sensitive financial data
...
-- 3. Intelligent Security Event Filtering
...
-- 4. Automated Security Event Cleanup

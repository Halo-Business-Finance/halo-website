...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Verify admin session using RPC function (same as admin-leads)
    const { data: adminId, error: sessionError } = await supabase.rpc('verify_admin_session', { token })

    if (sessionError || !adminId) {

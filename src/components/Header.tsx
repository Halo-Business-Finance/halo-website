    ...
    <header className="relative bg-white shadow-lg border-b border-slate-200/60 z-50">
      ...
      <nav className="container mx-auto px-4" aria-label="Main navigation">
        <div className="flex items-center justify-between h-20">
          ...
          {/* Desktop Navigation - JPMorgan/Chase inspired */}
          <div className="hidden lg:flex items-center space-x-1">
            {Object.entries(menuItems).map(([key, item]) => (
              <DropdownMenu key={key}>
                <DropdownMenuTrigger className="flex items-center px-4 py-3 text-slate-700 hover:text-financial-blue font-medium transition-colors duration-200 rounded-lg hover:bg-slate-50 group">
                  {item.title}
                  <ChevronDown className="ml-1 h-4 w-4 transition-transform duration-200 group-hover:rotate-180" />
                </DropdownMenuTrigger>
                <DropdownMenuContent className="bg-white border border-slate-200 shadow-[var(--shadow-professional)] rounded-xl p-2 min-w-[240px]">
                  {item.items.map((subItem) => (
                    <DropdownMenuItem key={subItem} className="rounded-lg hover:bg-slate-50 transition-colors duration-200 p-3" asChild>
                      <Link to={getItemLink(item.title, subItem)} className="flex items-center text-slate-700 hover:text-financial-blue font-medium">
                        {subItem}
                      </Link>
                    </DropdownMenuItem>
                  ))}

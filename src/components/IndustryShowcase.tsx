...
const IndustryShowcase = ({ className }: IndustryShowcaseProps) => {
  ...
  const nextSlide = () => {
    ...
    const maxIndex = window.innerWidth < 768 ? industries.length - 1 : Math.max(0, industries.length - 4);
  ...
  return <section className="py-16 bg-[hsl(0_0%_100%)]">
      <div className="container mx-auto px-4">
        ...
        <div className="md:hidden relative mb-12">
                  ...
                  <Card className="overflow-hidden group hover:shadow-lg transition-all duration-300 h-full">
                    ...
                    <CardContent className="p-6">
                      ...
                      <ul className="space-y-2 mb-4">
                        {industry.loanTypes.map((type, i) => <li key={i} className="text-xs flex items-center">
          ...
          <div className="flex justify-center space-x-2 mt-4">
            {industries.map((_, index) => <button key={index} onClick={() => setCurrentIndex(index)} className={`w-2 h-2 rounded-full transition-all duration-200 ${index === currentIndex ? 'bg-primary scale-110' : 'bg-gray-300 hover:bg-gray-400'}`} />)}
          </div>
        </div>
        ...
        <div className="hidden md:block relative mb-12">
          <div className="overflow-hidden">
            ...
            transform: `translateX(-${currentIndex * 25}%)`
              ...
              {industries.map((industry, index) => <div key={index} className="w-1/4 flex-shrink-0 px-3 border-0 border-black">
                  <Card className="overflow-hidden group hover:shadow-lg transition-all duration-300 h-full">
                    <div className="relative h-48 overflow-hidden">
                      <img src={industry.image} alt={industry.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
                      <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent" />

...
const TrustSignals = () => {
  const stats = [
    { icon: DollarSign, value: '$2.5B+', label: 'Total Funded', color: 'text-emerald-500' },
    { icon: Users, value: '15,000+', label: 'Businesses Helped', color: 'text-blue-500' },
    { icon: Clock, value: '24hrs', label: 'Avg. Pre-Approval', color: 'text-amber-500' },
    { icon: Award, value: '98%', label: 'Client Satisfaction', color: 'text-purple-500' },
  ];
  ...
  const partners = [
    { name: 'SBA', icon: Building2 },
    { name: 'FDIC', icon: Shield },
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6 mb-16">
          {stats.map((stat, index) => (
            <Card 
              key={index} 
              className="group bg-white/80 backdrop-blur-sm border-border/50 hover:shadow-lg hover:scale-105 transition-all duration-300 cursor-default"
            >
        ...
        <div className="mb-16">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {testimonials.map((testimonial, index) => (
              <Card 
                key={index} 
                className="group bg-white border-border/50 hover:shadow-xl hover:-translate-y-1 transition-all duration-300"
              >
        ...
        <div className="text-center">
          <p className="text-sm text-muted-foreground mb-6">Trusted & Certified</p>
          <div className="flex justify-center items-center gap-8 flex-wrap">
            {partners.map((partner, index) => (
              <div 
                key={index}
                className="flex items-center gap-2 px-4 py-2 bg-white rounded-lg border border-border/50 hover:shadow-md transition-shadow duration-300"
              >

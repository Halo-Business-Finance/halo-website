...
import { Building2, Truck, Stethoscope, ShoppingBag, Utensils, Factory, ChevronRight, ChevronLeft, DollarSign, Clock, CheckCircle2 } from 'lucide-react';
...
const FinancingSolutions = () => {
  ...
  const handleNext = () => {
    if (step === 1 && selectedIndustry) setStep(2);else if (step === 2 && selectedAmount) setStep(3);
  };
  const handleBack = () => {
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="text-center mb-16">
          <div className="inline-flex items-center gap-2 bg-primary/10 border border-primary/20 rounded-full px-4 py-2 mb-6">
            ...
            <span className="text-sm font-medium text-white">Tailored Business Solutions</span>
          </div>
          <h2 className="text-3xl font-bold text-white mb-4 md:text-3xl">
          ...
          <div className="md:hidden relative">
            <div className="overflow-hidden">
              ...
              transform: `translateX(-${currentIndex * 100}%)`
                ...
                {industries.map((industry, index) => <div key={index} className="w-full flex-shrink-0 px-3">
                    <Card className="overflow-hidden group hover:shadow-xl transition-all duration-300 h-full border-slate-700 bg-slate-800">
                      ...
                      <CardContent className="p-6 bg-slate-800">
                        ...
                        <ul className="space-y-2 mb-4">
                          {industry.loanTypes.map((type, i) => <li key={i} className="text-xs text-slate-400 flex items-center">
            ...
            <div className="flex justify-center space-x-2 mt-4">
              {industries.map((_, index) => <button key={index} onClick={() => setCurrentIndex(index)} className={`w-2 h-2 rounded-full transition-all duration-200 ${index === currentIndex ? 'bg-primary scale-110' : 'bg-slate-600 hover:bg-slate-500'}`} />)}
            </div>
          </div>
              ...
              transform: `translateX(-${currentIndex * 25}%)`
                ...
                {industries.map((industry, index) => <div key={index} className="w-1/4 flex-shrink-0 px-3">
                    <Card className="overflow-hidden group hover:shadow-xl hover:-translate-y-1 transition-all duration-300 h-full border-slate-700 bg-slate-800">
                      ...
                      <CardContent className="p-6 bg-slate-800">
                        <p className="text-sm mb-4 leading-relaxed text-white">{industry.description}</p>
                        <ul className="space-y-2 mb-4">
                          {industry.loanTypes.map((type, i) => <li key={i} className="text-xs flex items-center text-white">
                              <div className="w-1.5 h-1.5 rounded-full mr-2 flex-shrink-0 text-white bg-white" />
                              {type}
                            </li>)}
                        </ul>
                        <Button asChild size="sm" className="w-full bg-primary hover:bg-primary/90">
        ...
        <div className="flex items-center gap-6 mb-16">
          ...
          <div className="flex items-center gap-3 bg-transparent">
            ...
            <span className="text-sm font-medium uppercase tracking-wider text-white">Find Your Loan</span>
            <div className="w-2 h-2 rounded-full bg-primary" />
          </div>
        ...
        <div className="max-w-3xl mx-auto">
          {step === 1 && <Card className="border-slate-700 bg-slate-800/50 backdrop-blur">
              <CardHeader>
                <div className="mb-4">
              ...
              <CardContent>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  {quizIndustries.map(industry => <button key={industry.id} onClick={() => setSelectedIndustry(industry.id)} className={`p-4 rounded-lg border-2 transition-all duration-200 flex flex-col items-center gap-2 hover:shadow-md ${selectedIndustry === industry.id ? 'border-primary bg-primary/10' : 'border-slate-600 hover:border-primary/50 bg-slate-700/50'}`}>
          ...
          {step === 2 && <Card className="border-slate-700 bg-slate-800/50 backdrop-blur">
              <CardHeader>
                <CardTitle className="text-xl text-white">How much funding do you need?</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="grid grid-cols-2 gap-4">
                  {amounts.map(amount => <button key={amount.id} onClick={() => setSelectedAmount(amount.value)} className={`p-6 rounded-lg border-2 transition-all duration-200 flex flex-col items-center gap-2 hover:shadow-md ${selectedAmount === amount.value ? 'border-primary bg-primary/10' : 'border-slate-600 hover:border-primary/50 bg-slate-700/50'}`}>
          ...
          {step === 3 && <div className="space-y-6">
              ...
              <div className="grid md:grid-cols-2 gap-6">
                {getRecommendations().map(loan => <Card key={loan.id} className="border-slate-700 bg-slate-800 hover:shadow-xl hover:-translate-y-1 transition-all duration-300">

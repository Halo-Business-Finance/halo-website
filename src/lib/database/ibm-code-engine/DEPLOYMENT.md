...
ibmcloud cr namespace-add hbf-capital

# Build and push image
docker build -t us.icr.io/hbf-capital/hbf-api:latest .
docker push us.icr.io/hbf-capital/hbf-api:latest
```
...
```bash
# Create project
ibmcloud ce project create --name hbf-api

# Select project
ibmcloud ce project select --name hbf-api
```
...
ibmcloud ce secret create --name hbf-db-credentials \
...
ibmcloud ce secret create --name hbf-appid-credentials \
...
ibmcloud ce app create --name hbf-api \
  --image us.icr.io/hbf-capital/hbf-api:latest \
  --port 8080 \
  --min-scale 1 \
  --max-scale 10 \
  --cpu 0.5 \
  --memory 1G \
  --env-from-secret hbf-db-credentials \
  --env-from-secret hbf-appid-credentials \
  --env ALLOWED_ORIGINS=https://hbfcapital.com,https://www.hbfcapital.com
```

## Step 5: Get Application URL

```bash
ibmcloud ce app get --name hbf-api --output url
```
...
| `ALLOWED_ORIGINS` | Comma-separated allowed origins | `https://hbfcapital.com` |
...
# Update application
ibmcloud ce app update --name hbf-api --image us.icr.io/hbf-capital/hbf-api:latest
```

## Viewing Logs

```bash
ibmcloud ce app logs --name hbf-api --follow
```
...
```bash
ibmcloud ce secret update --name hbf-db-credentials \
...
```env
VITE_IBM_CODE_ENGINE_URL=https://hbf-api.xxx.us-south.codeengine.appdomain.cloud
```
